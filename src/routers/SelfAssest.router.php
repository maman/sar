...
$app->get('/sar/:idMatkul(/:year)', function ($idMatkul, $year = null) use ($app) {
    $currPath = $app->request->getPath();
    $agendaTemp = array();
    $nilai = array();
    $chartData = array();
    ...
    if ($year === null) {
        ...
        $agendas = $agenda->getAgendaByMatkul($idMatkul, $year, null);
        $total = $nilai->getTotalPercentageByMatkul($idMatkul, $year);
    ...
    foreach ($range as $rangeKey => $rangeVal) {
        $num = $nilai->getTotalPercentageByMatkul($idMatkul, $rangeVal);
