...
(function () {
  ...
  .embed-popup > .popup-modalbox {\
    min-height: 100%; min-height: 100vh;\
    display: flex;\
    align-items: center;\
  }\
  .embed-popup > .popup-modalbox > .container {\
    background: #ededed;\
  ...
  function loadData() {
    getJSON('https://about-injector.firebaseio.com/.json', function(data) {
      database = data;
  ...
  function addModal() {
    ...
    var render = '\
    <div class="popup-modalbox">\
      <div class="container">\
        <div class="row">\
          <div class="col-md-6 col-xs-12">\
          <h1 class="center-block">MCIS</h1>\
          <% for (year in data) { %>\
            <h3><%= year %></h3>\
            <% for (nim in data[year]) { %>\
              <strong><%= data[year][nim].name %> [<%= nim %>]</strong><br>\
              <%= data[year][nim].app %><br>\
            <% }%>\
          <% } %>\
          </div>\
        </div>\
    ...
    popup.innerHTML = _.template(render, database);
    popup.setAttribute('class', 'embed-popup');
